...
For now, when using the build with GPU support, AVX2 will be disabled even when available (due to a limitation of NVCC itself)
so some CPU operations may be slower.
